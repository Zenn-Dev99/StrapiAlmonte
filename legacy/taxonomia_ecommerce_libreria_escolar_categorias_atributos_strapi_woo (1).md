...
## 12) Ficha técnica de productos y despacho (peso/medidas/cálculos)

> Objetivo: centralizar en **Strapi** los datos técnicos necesarios para ficha de producto y para cálculo de despacho (tarifa por peso real/volumétrico), con extensibilidad por tipo de producto.

### 12.1. Diseño de datos (recomendado)

- **Campos comunes** (en `Product` o componente `techSpecs`):
  - `weightKg` *(number, >=0)* — **peso real** del producto **unitario** (kg).
  - `lengthCm`, `widthCm`, `heightCm` *(number, >=0)* — **medidas unitarias** (cm), **sin empaque** si vendes suelto; con empaque si vendes sellado.
  - `packageLengthCm`, `packageWidthCm`, `packageHeightCm` — medidas **empaque** (cm) para despacho (opcional pero recomendado).
  - `unitsPerPack` *(integer, >=1, default 1)* — cuántas unidades caben en un paquete de despacho.
  - `manufacturer` *(string)*, `brand` *(string)*, `model` *(string opcional)*.
  - `pages` *(integer, >=1, solo libros)*.
  - `material` *(string, mochilas/cuadernos)*, `gsm` *(paper gramaje)*.
  - `hazmat` *(boolean, default false)* — material peligroso/restricción transporte.
  - `returnable` *(boolean)* — admite devolución.
  - `originCountry` *(string enum ISO)*.
  - `hsCode` *(string)* — código arancelario si exportas.
- **Cálculos útiles** (persistidos para performance):
  - `volumeCm3` = `lengthCm * widthCm * heightCm`.
  - `packageVolumeCm3` = `packageLengthCm * packageWidthCm * packageHeightCm`.
  - `volumetricWeightKg` = `packageVolumeCm3 / divisorAereo` (p.ej. **5000** para cm/kg, depende del carrier; ver 12.4).
  - `billableWeightKg` = `max(weightKg, volumetricWeightKg)`.

> **Nota**: si un producto se vende en **packs**, considerar un **componente de variante** (ej. `packOf` = 6/10/12) y multiplicar estos campos en el carrito.

### 12.2. Componentes en Strapi

**A) Componente base `techSpecs`** (para todos)

`src/components/specs/tech-specs.json`
```json
{
  "collectionName": "components_specs_tech_specs",
  "info": {"displayName": "techSpecs"},
  "attributes": {
    "weightKg": {"type":"decimal"},
    "lengthCm": {"type":"decimal"},
    "widthCm": {"type":"decimal"},
    "heightCm": {"type":"decimal"},
    "packageLengthCm": {"type":"decimal"},
    "packageWidthCm": {"type":"decimal"},
    "packageHeightCm": {"type":"decimal"},
    "unitsPerPack": {"type":"integer","default": 1},
    ...
    "volumeCm3": {"type":"biginteger"},
    "packageVolumeCm3": {"type":"biginteger"},
    "volumetricWeightKg": {"type":"decimal"},
    "billableWeightKg": {"type":"decimal"}
  }
}
```

**B) Extensión por tipo** (suma al `techSpecs`)
- `bookSpecs` ya definido (añadir `pages` si no estaba):
```json
{
  "pages": {"type":"integer"}
}
```
- `notebookSpecs`: `size`, `ruling`, `sheets`, `gsm`.
- `paperSpecs`: `size`, `type`, `gsm`.
- `bagSpecs`: `capacityLiters`, `material`.
- `calcSpecs`: `type`, `power`, `functionsCount`.

**En `Product`** agregar `specs` como **dynamic zone** y **añadir `specs.techSpecs` como primer componente** obligatorio:
```json
{
  "attributes": {
    "specs": {
...
### 12.3. Lifecycles (cálculo automático)

`src/api/product/content-types/product/lifecycles.js`
```js
const calc = (data) => {
  const s = Array.isArray(data.specs) ? data.specs.find(c => c.__component === 'specs.tech-specs') : null;
  if (!s) return;
  const L = Number(s.packageLengthCm || s.lengthCm || 0);
  const W = Number(s.packageWidthCm  || s.widthCm  || 0);
  const H = Number(s.packageHeightCm || s.heightCm || 0);
  const weight = Number(s.weightKg || 0);
  const divisor = Number(process.env.VOLUMETRIC_DIVISOR_CMKG || 5000); // cm³ por kg

  const vol = (Number(s.lengthCm||0) * Number(s.widthCm||0) * Number(s.heightCm||0)) || 0;
  const packVol = L * W * H;
  const volW = packVol && divisor ? (packVol / divisor) : 0;
  const billable = Math.max(weight, volW);

  s.volumeCm3 = Math.round(vol);
  s.packageVolumeCm3 = Math.round(packVol);
  s.volumetricWeightKg = Number(volW.toFixed(3));
  s.billableWeightKg = Number(billable.toFixed(3));
};

module.exports = {
  beforeCreate: async (event) => calc(event.params.data),
  beforeUpdate: async (event) => calc(event.params.data)
};
```

> **ENV**: agrega `VOLUMETRIC_DIVISOR_CMKG=5000` en Railway; si tu carrier usa otro (ej. 6000), cámbialo sin redeploy.

### 12.4. Reglas de negocio de despacho

- **Peso cobrado** = `max(peso real, peso volumétrico)`.
- **Divisor volumétrico** (cm³/kg) comunes: **5000** (aéreo internacional), **6000** (algunos couriers), **4000** (histórico). Confirma con transportista chileno.
- **Tabla de tramos** por región/comuna (Strapi Collection `shippingZone` opcional):
  - `zoneName`, `regionIds`, `communes`, `basePrice`, `perKg`, `minPrice`, `freeOver`.
- **Sobrecargos**: `hazmat`, `sobre-dimensiones` (si alguna medida > 70 cm, por ejemplo), **rural**.

### 12.5. Validaciones de datos (UI Content-Type Builder)
- `weightKg`, medidas y `pages` ≥ 0 (o ≥1 donde aplica).
- `unitsPerPack` ≥ 1.
- Si faltan `package*`, usar `length/width/height` para cálculo (fallback en lifecycle).
...
```json
{
  "type": "book",
  "specs": [
    {"__component":"specs.tech-specs","weightKg":0.48,"lengthCm":28,"widthCm":21,"heightCm":2,"unitsPerPack":1,"manufacturer":"Moraleja","brand":"Moraleja"},
    {"__component":"specs.book-specs","isbn":"978-956-000000-1","author":"Equipo Moraleja","editorial":"Moraleja","pages":380}
  ]
}
```
...
```json
{
  "type": "notebook",
  "specs": [
    {"__component":"specs.tech-specs","weightKg":0.32,"packageLengthCm":28,"packageWidthCm":22,"packageHeightCm":1.5,"unitsPerPack":10,"brand":"XBrand"},
    {"__component":"specs.notebook-specs","size":"A4","ruling":"universitario","sheets":100,"gsm":75}
  ]
}
```

### 12.7. Frontend (Next.js) — cálculo en carrito

```ts
export function volumetricKg({ L, W, H, divisor = 5000 }: { L:number; W:number; H:number; divisor?:number }) {
  return +( (L*W*H) / divisor ).toFixed(3);
}

export function billableKg({ weightKg, L, W, H, divisor }: { weightKg:number; L:number; W:number; H:number; divisor:number }) {
  return Math.max(weightKg, volumetricKg({ L, W, H, divisor }));
}
```

### 12.8. WooCommerce (mapeo)
- Woo usa `weight`, `length`, `width`, `height` nativos. Mapéalos desde `techSpecs` para sincronía.
- Para **volumétrico** y **billable**, puedes mantenerlos solo en Strapi o escribir a **meta fields** de Woo (`_vol_weight`, `_billable_weight`).

### 12.9. Checklist
- [ ] Crear componente `techSpecs` y sumarlo a `specs` (dynamic zone) en `Product`.
- [ ] Agregar lifecycle de cálculo y `VOLUMETRIC_DIVISOR_CMKG` en Railway.
- [ ] Definir `shippingZone` si calculas en backend.
- [ ] Validar productos clave (PAES, cuadernos, mochilas) con mediciones reales.
- [ ] Probar carrito con productos mixtos y verificar `billableWeightKg`.
